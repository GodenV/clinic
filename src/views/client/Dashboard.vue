<template>
  <div>
    ...
    <div v-if="!(isCreateMedicalCard && isCreateCustomer)">
      <section class="section" style="padding-top: 2rem !important">
        <b-steps v-model="step" size="is-medium" :has-navigation="false">
          <b-step-item label="Личная информация пациента" icon="user" />
          <b-step-item label="Данные карточки пациента" icon="info" />
        </b-steps>
      </section>
                  ...
                  <div class="stats-block">
                    ...
                    <div class="stat">
                      ...
                      <span class="is-size-5">{{
                        genders[customer.medicalCard.gender]
                      }}</span>
                    </div>
                  </div>
                  <div class="stats-block">
                    <div class="stat">
                      <span>Рост</span>
                      <span class="is-size-5"
                        >{{ customer.medicalCard.height }}см</span
                      >
                    </div>
                    <div class="stat">
                      <span>Вес</span>
                      <span class="is-size-5"
                        >{{ customer.medicalCard.weight }}кг</span
                      >
                    </div>
                    <div class="stat">
                      <span>Дата рождения</span>
                      <span class="is-size-5">{{
                        customer.medicalCard.birthday
                      }}</span>
                    </div>
                  </div>
                  <div class="contact-block">
            ...
            >
              <template slot-scope="props">
                ...
                <b-table-column field="name" label="ФИО" searchable sortable>
                  {{
                    props.row.doctorDto
                      ? props.row.doctorDto.surname +
                        " " +
                        props.row.doctorDto.name +
                        " " +
                        props.row.doctorDto.lastName
                      : "Не занят"
                  }}
                </b-table-column>
...
export default {
  ...
  methods: {
    ...
    connectToCall() {
      this.$router.push({ name: "chat", query: { role: "client" } });
    },
  },
  created() {
    this.reloadData();
