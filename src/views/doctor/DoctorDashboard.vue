  ...
  <div>
    ...
    <section class="container">
      <div class="columns">
        <div class="columns is-multiline column is-5">
          <div class="column is-12 doctor-card">
            <div
              class="flex-card is-avatar light-bordered card-overflow no-padding-top"
            >
              <div class="inner">
                <div class="contact-block">
                  <div
                    class="align-left contact-name no-margin-top border-bottom"
                  >
                    <div>
                  ...
                  <div class="stats-block">
                    <div class="stat">
                      ...
                      <span class="is-size-5">{{
                        categories[doctor.medicalCategory]
                      }}</span>
                    </div>
                    <div class="stat">
            ...
            <div class="columns column no-margin is-6"></div>
          </div>
          <div class="has-text-centered column is-6">
            <b-button class="is-success" @click="isComponentModalActive = true"
              >Добавить талоны</b-button
            >
          </div>
          <div class="has-text-centered column is-6">
            <b-button @click="onDeleteAppointment" type="is-danger"
              >Удалить просроченные</b-button
            >
          </div>
        </div>
        <div class="column has-text-centered is-7" style="min-height: 500px">
          ...
          <b-table
            ...
            :row-class="
              (row, index) => {
                ...
                if (row.customerDto) {
                  ...
                  if (
                    time < Date.now() &&
                    (row.recommendations || row.diagnosis)
                  )
                    return 'r-green';
                  if (
                    time < Date.now() &&
            ...
            :has-detailed-visible="
              (row) => {
                let time = new Date(row.dateTime);
                return row.customerDto && time < Date.now();
              }
            "
          >
            <template slot-scope="props">
              ...
              <b-table-column field="name" label="ФИО" searchable sortable>
                {{
                  props.row.customerDto
                    ? props.row.customerDto.surname +
                      " " +
                      props.row.customerDto.name +
                      " " +
                      props.row.customerDto.lastName
                    : "Не занят"
                }}
              </b-table-column>
              ...
              <b-table-column class="has-text-centered" label="Редактировать">
                <b-button
                  ...
                  v-show="
                    props.row.customerDto && props.row.dateTime > Date.now()
                  "
              ...
              <b-table-column class="has-text-centered" label="Удалить">
                <b-button
                  type="is-danger"
                  @click="onDeleteAppointment(props.row.id)"
                  v-if="!props.row.customerDto"
                >
                  Отменить
                </b-button>
              </b-table-column>
              <b-table-column class="has-text-centered" label="Дополнительно">
                <b-button
                  type="is-danger"
                  @click="connectToCall()"
                  v-if="props.row.customerDto"
      ...
      >
        <Appointment
          :doctorId="doctor.id"
          :afterSubmit="getAppointmentsList"
        ></Appointment>
      </b-modal>
      <b-modal
      ...
      >
        <div class="modal-card" style="width: auto">
          ...
          <section class="modal-card-body">
            ...
            <b-field label="Рекомендации">
              <b-input
                maxlength="200"
                type="textarea"
                v-model="appForm.recommendations"
              ></b-input>
            </b-field>
          </section>
          <footer class="modal-card-foot">
            ...
            <button
              @click="onSubmitApplicationUpdate"
    ...
    <BLoading :is-full-page="true" :active.sync="loading" :can-cancel="false" />
  </div>
</template>
...
export default {
  ...
  methods: {
    onSubmitApplicationUpdate() {
      updateAppointment(this.appForm.id, this.appForm).then(() => {
    ...
    getDepartment() {
      this.loading = true;
      getDepartment(this.doctor.departmentId)
        .then((response) => {
    ...
    getAppointmentsList() {
      ...
      getAppointments(
        this.doctor.id,
        dayjs(this.filterDate).format("YYYY-MM-DD")
      )
        .then((response) => {
    ...
    connectToCall() {
    this.$router.push({ name: "chat", query: { role: "doctor" } });
  },
  },
  created() {
    this.getCurrentDoctor().then(() => {
      this.getAppointmentsList();
      this.getDepartment();
    });
  },
...
.r-blue {
  background-color: rgb(74, 244, 250);
}
